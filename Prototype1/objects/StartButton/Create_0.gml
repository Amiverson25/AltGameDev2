/// @description Insert description here
// You can write your code in this editor

audio_play_sound(StartMusic, 0, false);
globalvar fishyy;
globalvar smallFish;
globalvar mediFish;
globalvar largFish;
globalvar savings;
globalvar pollution;
globalvar pierManDialog;
globalvar dialogTownMan;
globalvar day;
globalvar day2Dialog;
globalvar dialog4;
globalvar day3TalkedToBoat;
globalvar day5TalkedToBoat;
globalvar talkedToHomeless;// = 0
globalvar acceptedHomeless;// = 0;


global.talkedToHomeless = 0;
global.acceptedHomeless = 0;


global.day2Dialog = 1;
global.day3Dialog = 1;


global.nomasNets = 0;
global.dialog4 = 0;

global.day = 1;
global.day3TalkedToBoat = 0;
global.day5TalkedToBoat = 0;
global.pollution = 0;
global.caughtsickfish = 0;
global.savings = 1500;
global.new_total_savings = 0;
global.smallFish = 0;
global.mediFish = 0;
global.largFish = 0;
global.fishyy = 0; //still doesn't make sense
global.caughtTrash = 0;

global.pierManDialog = 1;
global.dialogTownMan = 1;

global.shop = 0;
global.rent = 700;
global.gas = 100;
global.electric = 100;
global.Food = 100;
global.totalexpenses = 0;
global.nets = 0;


