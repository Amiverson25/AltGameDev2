/// @description Insert description here
// You can write your code in this editor


//Draw Text Box
//draw_sprite(mailbox_dialog, 0, x, y);

//DrawNineSlice(mailbox_dialog, 0, x, y, x+width, y+height);

//draw text
//draw_set_font(fntDialogBubble);

//draw_text_ext(x, y, text, stringHeight, boxWidth);


//width = sprite_width;

//var sprWidth = sprite_get_width(sprite_index);
//var sprHeight = sprite_get_height(sprite_index);
//var margin = 8;
//var innerWidth = width - (2 * margin) - (2 * sprWidth/3);
//var lineHeight = 40;
//var innerHeight = string_height_ext(text, lineHeight, innerWidth);
//height = innerHeight + 2*margin + (2*sprHeight/3);

//DrawNineSlice(sprite_index, 0, x, y, x+width, y+height);

//draw_set_color(c_black);
//draw_set_font(fntDialogBubble);
//draw_set_halign(fa_center);
//draw_set_valign(fa_top);

//draw_text_ext(x + width/2, y+20, text, lineHeight, innerWidth);